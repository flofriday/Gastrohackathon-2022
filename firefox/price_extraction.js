function replacePrices() {
    ...
    var boxes = document.querySelectorAll('#ams-search-result-list > sn-list-item-container');
    var counter = 0;
    for (let single_box of boxes) {
        var link = (single_box.getElementsByTagName('a')[0]).href;
        if (correct_url_regex.test(link)) {
            ...
            req.send(null);
            if (req.status == 200) {
                var loadedText = req.responseText;
